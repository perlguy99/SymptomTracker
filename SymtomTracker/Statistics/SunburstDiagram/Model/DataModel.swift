...
extension SunburstConfiguration {
    ...
    private func validateAndPrepareValues() {
        ...
        case .parentIndependent(let totalValue):
            ...
            guard validateTotalValue(nodes: nodes, totalValue: totalValue) else {
                fatalError("The sunburst nodes, or the total value provided with the .parentIndependent CalculationMode is invalid. The total value cannot be less than the sum of the nodes. \(nodes) : \(totalValue!)")
            }
        }
    }
